...
export function renderOverlay(opts: {
  ...
  // Everything below is ONE template literal, so a backtick anywhere in it
  // -- including inside a YAML comment, quoting an identifier the way prose
  // wants to -- closes the string and turns the rest of the file into
  // syntax errors. It has happened twice. Use quotes in comments here.
...
${
  ...
  minio:
    # quay, not Docker Hub. The minio/minio repository is GONE from Docker
    # Hub -- it 404s outright, not just this tag -- so aggregator-dpg's own
    # local-setup cannot boot from a fresh clone either. quay.io is MinIO's
    # other published registry and carries the same releases.
